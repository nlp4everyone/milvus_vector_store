# Typing
from typing import Optional, Union, List, Sequence, Literal, Tuple
...
# Component
import numpy, uuid, pathlib, mimetypes
# Pymilvus
...
class ImageMilvusVectorStore(BaseVectorStore):
    def __init__(self,
                 image_embedding_model: ImageEmbedding,
                 text_embedding_model :Optional[BaseEmbedding] = None,
    ...
    def insert_images(self,
                      ...
                      uploading_batch_size :Optional[int] = None,
                      **kwargs):
        """
        ...
        if isinstance(images[0],str):
            # Path case
            # Filter only images
            images_url, images_path = self.__get_image_path(file_paths = images)
            # Get mimetype
            images_mimetype = self.__get_image_mimetype(file_paths = images_url)

            # Get encodings
            image_embeddings = self._encode_images(images = images_url,
            ...
            image_nodes = [ImageNode(id_ = str(uuid.uuid4()),
                                     ...
                                     embedding = image_embeddings[i].tolist()).dict() for i in range(len(images_url))]
    ...
    def retrieve(self,
        ...
        if is_image:
            # Image convert
            query_embeddings = self._encode_images(images = queries,
                                                   embedding_model = self._image_embedding_model,
                                                   batch_size = 2,
                                                   parallel = None)
            ...
            # Text query case
            raise NotImplementedError("This version doesnt support text for querying!")
    ...
    def _encode_images(self,
        ...
        return list(self._image_embedding_model.embed(images = images,
                                                      batch_size = batch_size,
                                                      parallel = parallel,
                                                      **kwargs))

    @staticmethod
    def __get_image_path(file_paths: List[str]) -> Tuple[List[str], List[str]]:
        # Image extension
    ...
    @staticmethod
    def __get_image_mimetype(file_paths: List[str]) -> List[Union[str,None]]:
        # Return
        return [mimetypes.guess_type(pathlib.Path(url))[0] for url in file_paths]
    ...
    def __convert_retrieval_nodes(self,
        ...
        for result in results:
            # Append
            final_output.append([ImageNode.parse_obj(dict(element).get("entity")) for element in result])
        return final_output
